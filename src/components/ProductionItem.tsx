...
const ProductionItem = ({
  production, isEditing, editFormState,
  onStartEditing, onCancelEditing, onUpdateProduction, onDeleteProduction, setEditFormState,
}: ProductionItemProps) => {
  if (isEditing) {
    return (
      <tr>
        ...
        <td colSpan={3}>
          <form onSubmit={onUpdateProduction} style={{ display: 'flex', gap: '10px', width: '100%' }}>
            <input type="number" value={editFormState.quantity_produced}
            ...
            <input type="date" value={editFormState.production_date}
    ...
    <tr>
      ...
      <td>
        <div className="button-group">
          ...
          <button className="button button-secondary" onClick={() => onDeleteProduction(production._id)}>Eliminar</button>
        </div>
      </td>
    </tr>
  );
