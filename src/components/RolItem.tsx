...
const RolItem = ({
  role, isEditing, editFormState,
  onStartEditing, onCancelEditing, onUpdateRole, onDeleteRole, setEditFormState,
}: RolItemProps) => {
  if (isEditing) {
    return (
      <tr>
        <td>
          <form onSubmit={onUpdateRole} style={{ display: 'flex', gap: '10px', width: '100%' }}>
            <input type="text" value={editFormState.rol_name}
              onChange={(e) => setEditFormState({ ...editFormState, rol_name: e.target.value })}
              placeholder="Nombre del Rol" required style={{ flex: 1 }} />
    ...
    <tr>
      ...
      <td>
        <div className="button-group">
          ...
          <button className="button button-secondary" onClick={() => onDeleteRole(role._id)}>Eliminar</button>
        </div>
      </td>
    </tr>
  );
