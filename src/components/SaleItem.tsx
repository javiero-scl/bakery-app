...
const SaleItem = ({
  sale, isEditing, editFormState,
  onStartEditing, onCancelEditing, onUpdateSale, onDeleteSale, setEditFormState,
}: SaleItemProps) => {
  if (isEditing) {
    return (
      <tr>
        ...
        <td colSpan={4}>
          <form onSubmit={onUpdateSale} style={{ display: 'flex', gap: '10px', width: '100%' }}>
            <input type="number" value={editFormState.quantity_sold}
            ...
            <input type="number" value={editFormState.unit_sale_price}
              onChange={(e) => setEditFormState({ ...editFormState, unit_sale_price: parseFloat(e.target.value) })}
              placeholder="Precio Unitario" required style={{ flex: 1 }} />
            <input type="date" value={editFormState.sale_date}
    ...
    <tr>
      ...
      <td>
        <div className="button-group">
          ...
          <button className="button button-secondary" onClick={() => onDeleteSale(sale._id)}>Eliminar</button>
        </div>
      </td>
    </tr>
  );
