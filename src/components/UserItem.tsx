...
import { AppUser } from '../pages/Users';

interface UserItemProps {
  user: AppUser;
  isEditing: boolean;
  editFormState: { user_name: string; user_email: string };
  onStartEditing: (user: AppUser) => void;
...
const UserItem = ({
  user, isEditing, editFormState,
  onStartEditing, onCancelEditing, onUpdateUser, onDeleteUser, setEditFormState,
}: UserItemProps) => {
  if (isEditing) {
    return (
      <tr>
    ...
    <tr>
      ...
      <td>
        <div className="button-group">
          ...
          <button className="button button-secondary" onClick={() => onDeleteUser(user._id)}>Eliminar</button>
        </div>
      </td>
    </tr>
  );
